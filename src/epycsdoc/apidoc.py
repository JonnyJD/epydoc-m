# epycsdoc -- API Documentation Classes
#
...
import epycsdoc
import __builtin__
from epycsdoc.compat import * # Backwards compatibility
from epycsdoc.util import decode_with_backslashreplace, py_src_filename
import epycsdoc.markup.pyval_repr
...
class DottedName:
    ...
    a dotted name can be accessed using the indexing operator:
        ...
        >>> name = DottedName('epycsdoc', 'api_doc', 'DottedName')
        >>> print name
        epycsdoc.apidoc.DottedName
        >>> name[1]
        'api_doc'
    """
    UNREACHABLE = "??"
    ...
    def __str__(self):
        ...
        identifiers with periods:
            ...
            >>> print DottedName('epycsdoc', 'api_doc', DottedName')
            epycsdoc.apidoc.DottedName
        """
...
class APIDoc(object):
    ...
    """@ivar: A description of the documented item, extracted from its
       docstring.
       @type: L{ParsedDocstring<epycsdoc.markup.ParsedDocstring>}"""
    ...
    """@ivar: A list of new docstring fields tags that are defined by the
       ...
       @type: L{DocstringField <epycsdoc.docstringparser.DocstringField>}"""
    ...
    def _debug_setattr(self, attr, val):
        ...
        L{epycsdoc.DEBUG} is true, to make sure we don't accidentally
...
class ValueDoc(APIDoc):
    ...
    @see: L{epycsdoc.markup.pyval_repr}"""
    ...
    def pyval_repr(self):
        ...
        if not hasattr(self, '_ValueDoc__pyval_repr'):
            self.__pyval_repr = epycsdoc.markup.pyval_repr.colorize_pyval(
    ...
    def summary_pyval_repr(self, max_len=None):
        ...
        if max_len is not None:
            return epycsdoc.markup.pyval_repr.colorize_pyval(
                self.pyval, self.parse_repr, self.REPR_MIN_SCORE,
        ...
        if not hasattr(self, '_ValueDoc__summary_pyval_repr'):
            self.__summary_pyval_repr = epycsdoc.markup.pyval_repr.colorize_pyval(
                self.pyval, self.parse_repr, self.REPR_MIN_SCORE,
...
class NamespaceDoc(ValueDoc):
    ...
    """@ivar: The contents of the namespace, encoded as a
        ...
        <epycsdoc.docbuilder.inherit_docs>} has added them).
...
class RoutineDoc(ValueDoc):
    ...
    """@ivar: A list of descriptions of the routine's
       ...
       <epycsdoc.markup.ParsedDocstring>} describing the argument(s)
       specified by C{arg}.
       @type: C{list}"""
    arg_types = UNKNOWN
    """@ivar: Descriptions of the expected types for the
       ...
       <epycsdoc.markup.ParsedDocstring>}"""
    ...
    """@ivar: A list of descriptions of exceptions
       ...
       <epycsdoc.markup.ParsedDocstring>} describing the circumstances
