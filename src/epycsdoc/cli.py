# epycsdoc -- Command line interface
#
...
"""
Command-line interface for epycsdoc.  Abbreviated Usage::

 epycsdoc [options] NAMES...
     ...
     -V, --version             Print the version of epycsdoc.
     -h, --help                Display a usage message.

Run \"epycsdoc --help\" for a complete option list.  See the epycsdoc(1)
...
from epycsdoc.util import wordwrap, run_subprocess, RunSubprocessError
from epycsdoc.util import plaintext_to_html, TerminalController
from epycsdoc.apidoc import UNKNOWN
from epycsdoc.compat import *
import ConfigParser
from epycsdoc.docwriter.html_css import STYLESHEETS as CSS_STYLESHEETS
from epycsdoc.docwriter.latex_sty import STYLESHEETS as STY_STYLESHEETS
from epycsdoc.docwriter.dotgraph import DotGraph
from epycsdoc.docwriter.dotgraph import COLOR as GRAPH_COLOR

# This module is only available if Docutils are in the system
try:
    from epycsdoc.docwriter import xlink
except:
    xlink = None
...
def option_defaults():
    return dict(
        ...
        debug=epycsdoc.DEBUG, profile=False, graphs=[],
...
def parse_arguments():
    ...
    version = "Epydoc, version %s" % epycsdoc.__version__
    ...
    action_group.add_option("--version",
        action='callback', callback=add_action,
        help="Show epycsdoc's version number and exit.")
    ...
    generation_group.add_option('--include-log',
        ...
        help=("Include a page with the process log (epycsdoc-log.html)"))
    ...
    graph_group.add_option("--profile-epycsdoc",
        action="store_true", dest="profile",
        help=SUPPRESS_HELP or
             ("Run the hotshot profiler on epycsdoc itself.  Output "
...
def parse_configfiles(configfiles, options, names):
    ...
    for configfile in configfiles:
        ...
        fp.close()
    for optname in configparser.options('epycsdoc'):
        val = configparser.get('epycsdoc', optname, vars=os.environ).strip()
...
def main(options):
    ...
    @return: the L{epycsdoc.apidoc.DocIndex} object created while
        running epycsdoc (or None).
    ...
    from epycsdoc import docstringparser
    ...
    # Set the dot path
    if options.dotpath:
        from epycsdoc.docwriter import dotgraph
    ...
    if options.load_pickle:
        ...
        from epycsdoc.docbuilder import build_doc_index
...
def write_html(docindex, options):
    from epycsdoc.docwriter.html import HTMLWriter
...
def write_latex(docindex, options):
    ...
    from epycsdoc.docwriter.latex import LatexWriter, show_latex_warnings
    ...
    try:
        try:
            ...
            for latex_command in latex_commands:
                ...
                if options.verbosity > 2 or epycsdoc.DEBUG:
...
def write_text(docindex, options):
    ...
    from epycsdoc.docwriter.plaintext import PlaintextWriter
...
class ConsoleLogger(log.Logger):
    ...
    def log(self, level, message):
        ...
        elif epycsdoc.DEBUG and level == log.DEBUG:
...
class HTMLLogger(log.Logger):
    ...
    HEADER = textwrap.dedent('''\
        ...
        <body bgcolor="white" text="black" link="blue" vlink="#204080"
              alink="#204080">
        <h1 class="epycsdoc">Epydoc Log</h1>
