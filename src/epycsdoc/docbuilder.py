# epycsdoc -- Documentation Builder
#
...
objects.  These data structures are created using a series of steps:
  ...
  1. B{Building docs}: Extract basic information about the objects,
     ...
     introspecting the objects' values (with L{epycsdoc.docintrospecter}; or
     by parsing their source code (with L{epycsdoc.docparser}.
...
The main interface to C{epycsdoc.docbuilder} consists of two functions:
...
from epycsdoc.docintrospecter import get_value_from_filename, get_value_from_name
from epycsdoc.docparser import parse_docs, ParseError
from epycsdoc.docstringparser import parse_docstring
from epycsdoc import log
from epycsdoc.util import *
    ...
    @param item: The item to document, specified using any of the
        following:
          - A string, naming a python package directory
            (e.g., C{'epycsdoc/markup'})
          - A string, naming a python file
            (e.g., C{'epycsdoc/docparser.py'})
          - A string, naming a python object
            (e.g., C{'epycsdoc.docparser.DocParser'})
