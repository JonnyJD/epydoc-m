#
# epycsdoc -- HTML output generator
# Edward Loper
#
...
"""
The HTML output generator for epycsdoc.  The main interface provided by
...
import time, epycsdoc, epycsdoc.markup, epycsdoc.markup.epytext
from epycsdoc.docwriter.html_colorize import PythonSourceColorizer
from epycsdoc.docwriter import html_colorize
from epycsdoc.docwriter.html_css import STYLESHEETS
from epycsdoc.docwriter.html_help import HTML_HELP
from epycsdoc.docwriter.dotgraph import *
from epycsdoc import log
from epycsdoc.util import plaintext_to_html, is_src_filename
...
def compile_template(docstring, template_string,
                     output_function='out', debug=epycsdoc.DEBUG):
    """
...
class HTMLWriter:
    ...
    def __init__(self, docindex, **kwargs):
        ...
        @keyword css: The CSS stylesheet file.  If C{css} is a file
              ...
              L{epycsdoc.docwriter.html_css}, then that stylesheet will
        ...
        @keyword include_log: If true, the the footer will include an
              href to the page 'epycsdoc-log.html'.
    ...
    def write(self, directory=None):
        ...
        log.progress(self._files_written/self._num_files, 'epycsdoc.css')
        self.write_css(directory, self._css)

        # Write the Javascript file.
        self._files_written += 1
        log.progress(self._files_written/self._num_files, 'epycsdoc.js')
    ...
    def write_module(self, out, doc):
        ...
        out('<!-- ==================== %s ' % typ.upper() +
            'DESCRIPTION ==================== -->\n')
        out('<h1 class="epycsdoc">%s %s</h1>' % (typ, shortname))
    ...
    def write_sourcecode(self, out, doc, name_to_docs):
        ...
        # Source code listing
        out('<h1 class="epycsdoc">Source Code for %s</h1>\n' %
    ...
    def write_module_tree(self, out):
        ...
        out('<h1 class="epycsdoc">Module Hierarchy</h1>\n')
    ...
    def write_class_tree(self, out):
        ...
        out('<h1 class="epycsdoc">Class Hierarchy</h1>\n')
    ...
    def write_link_index(self, out, indices, title, url, index_by_section,
        ...
        if sections:
            ...
            for section in sorted(sections):
                ...
                out('<h2 class="epycsdoc"><a name="%s">%s</a></h2></td>\n' %
                    (section, section))
                out('<td valign="top">\n')
    ...
    def write_metadata_index(self, out, indices, field, title, typ):
        ...
        # Page title.
        out('<h1 class="epycsdoc"><a name="%s">%s</a></h1>\n<br />\n' %
            (field, title))
    ...
    def write_help(self, out):
        ...
        L{epycsdoc.docwriter.html_help}.
        """
    ...
    write_redirect_index = compile_template(
        ...
        <head>
          ...
          <link rel="stylesheet" href="epycsdoc.css" type="text/css"></link>
        </head>
        <body>
    ...
    def write_css(self, directory, cssname):
        ...
        L{epycsdoc.docwriter.html_css}), then use that stylesheet;
        otherwise, use the default stylesheet.

        @rtype: C{None}
        """
        filename = os.path.join(directory, 'epycsdoc.css')
    ...
    #////////////////////////////////////////////////////////////
    #{ 2.9. Javascript (epycsdoc.js)
    #////////////////////////////////////////////////////////////

    def write_javascript(self, directory):
        jsfile = open(os.path.join(directory, 'epycsdoc.js'), 'w')
    ...
    #: the given fully-qualified dotted name.  E.g., for epycsdoc,
    #: <redirect.html#epycsdoc.apidoc.UNKNOWN> redirects the user to
    #: <epycsdoc.apidoc-module.html#UNKNOWN>.
    REDIRECT_URL_JS = '''
    ...
    write_footer = compile_template(
        ...
        >>>   if self._include_log:
            <a href="epycsdoc-log.html">Generated by Epydoc
            $epycsdoc.__version__$
        >>>       if self._include_timestamp:
            on $time.asctime()$</a>
        >>>   else:
            Generated by Epydoc $epycsdoc.__version__$
        >>>       if self._include_timestamp:
            on $time.asctime()$
        >>>   #endif
            </td>
            <td align="right" class="footer">
              <a target="mainFrame" href="http://epycsdoc.sourceforge.net"
                >http://epycsdoc.sourceforge.net</a>
            </td>
          </tr>
        </table>
        >>> #endif
    ...
    write_function_details_entry = compile_template(
        ...
        <tr><td>
          <table width="100%" cellpadding="0" cellspacing="0" border="0">
          <tr valign="top"><td>
          <h3 class="epycsdoc">$self.function_signature(var_doc)$
    ...
    write_property_details_entry = compile_template(
        ...
        >>> self.write_table_header(out, "details")
        <tr><td>
          <h3 class="epycsdoc">$var_doc.name$</h3>
          $descr$
          <dl class="fields">
    ...
    def write_redirect_page(self, out):
        ...
        fully-qualified dotted name.  E.g., for epycsdoc,
        <redirect.html#epycsdoc.apidoc.UNKNOWN> redirects the user to
        <epycsdoc.apidoc-module.html#UNKNOWN>.
        """
...
class _HTMLDocstringLinker(epycsdoc.markup.DocstringLinker):
