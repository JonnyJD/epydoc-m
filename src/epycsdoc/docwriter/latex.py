#
# epycsdoc.py: epycsdoc LaTeX output generator
# Edward Loper
#
...
"""
The LaTeX output generator for epycsdoc.  The main interface provided by
...
from epycsdoc.apidoc import *
from epycsdoc.compat import *
import epycsdoc
from epycsdoc import log
from epycsdoc import markup
from epycsdoc.util import plaintext_to_latex
import epycsdoc.markup
from epycsdoc.docwriter.dotgraph import *
from epycsdoc.docwriter.latex_sty import STYLESHEETS

class LatexWriter:
    #: Expects (options, epycsdocc_sty_package)
    PREAMBLE = [
    ...
    def _write_sty(self, directory, stylesheet):
        ...
        copied to 'epycsdoc-default.sty', which makes it possible to
        reference it via \RequirePackage.
        """
        # Write all the standard style files
        for (name, sty) in STYLESHEETS.items():
            out = open(os.path.join(directory, 'epycsdoc-%s.sty' % name), 'wb')
            out.write(sty)
            out.close()

        # Default: use the 'epycsdoc-default' style.
        if stylesheet is None:
            self._epycsdocc_sty_package = 'epycsdoc-default'

        # Stylesheet name: use the indicated style.
        elif stylesheet in STYLESHEETS:
            self._epycsdocc_sty_package = 'epycsdoc-%s' % stylesheet

        # Custom user stylesheet: copy the style to epycsdoc-custom.
        elif os.path.exists(stylesheet):
            try: sty = open(stylesheet, 'rb').read()
            except: raise IOError("Can't open LaTeX style file: %r" %
                                  stylesheet)
            out = open(os.path.join(directory, 'epycsdoc-custom.sty'), 'wb')
            out.write(sty)
            out.close()
            self._epycsdocc_sty_package = 'epycsdoc-custom'
    ...
    def write_preamble(self, out):
        ...
        options = []
        options.append('creator={epycsdoc %s}' % epycsdoc.__version__)
        ...
        if 'restructuredtext' in epycsdoc.markup.MARKUP_LANGUAGES_USED:
            from epycsdoc.markup import restructuredtext
    ...
    def write_header(self, out, where):
        ...
        if isinstance(where, APIDoc):
            ...
            out('\n%% %s' % where)
        out('\n%%\n%% Generated by epycsdoc %s\n' % epycsdoc.__version__)
...
def show_latex_warnings(s):
    ...
    for m in LATEX_WARNING_RE.finditer(s):
        ...
        if m.group('file'):
            ...
            if block is not None: epycsdoc.log.end_block()
            epycsdoc.log.start_block(BLOCK % filename)
            block = filename
...
#                 log.debug(msg)
        ...
        # A package reported a warning:
            ...
            if (filestack[-1] is not None and
                ...
                epycsdoc.log.end_block()
                epycsdoc.log.start_block(BLOCK % filestack[-1])
