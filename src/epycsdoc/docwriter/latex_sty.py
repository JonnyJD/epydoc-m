# -*- latex -*-
#
# epycsdoc.css: LaTeX stylesheets (*.sty) for epycsdoc's LaTeX writer
#
# $Id: html_css.py 1717 2008-02-14 22:21:41Z edloper $
#

"""
LaTeX stylesheets (*.sty) for epycsdoc's LaTeX writer.
"""
...
% epycsdoc's latex writer (epycsdoc.docwriter.latex).  For more
% information, see the epycsdoc webpage.  This stylesheet accepts the
% following options:
%
...
\ProvidesClass{epycsdoc-base}[2008/02/26 v3.0.1 Epydoc Python Documentation]
...
% the same group (as specified by the epycsdoc @group field).  It is used
...
% groups (with the epycsdoc @group field), then groups are displayed
...
%     (from the epycsdoc @param, @arg, @kwarg, @vararg, @type fields)
%
%   - The function's return description (from the epycsdoc @rerturns field)
%
%   - The function's return type (from the epycsdoc @rtype field)
%
%   - The function's exceptions: An EpydocFunctionRaises list
%     environment describing exceptions that the function may raise
%     (from the epycsdoc @raises field)
%
...
% (From epycsdoc fields: @param, @arg, @kwarg, @vararg, @type)
...
% that can be raised by a function.  (From epycsdoc field: @raise)
...
%   - The variable's type (from the epycsdoc @type field)
...
%   - The property's type (from the epycsdoc @type field)
...
% epycsdoc is responsible for escaping them.  E.g., correct usage for a
...
% generated by epycsdoc 3.0.  Function lists are displayed using
% a boxedminipage for each function.  Variable and Property lists
% are displayed using a longtable, with a row for each object.
%
% $Id:$
\NeedsTeXFormat{LaTeX2e}
\ProvidesClass{epycsdoc-boxes}[2008/02/26 v3.0.1 Epydoc Python Documentation]
% Pass options to the epycsdoc base package.
\RequirePackage{xkeyval}
\DeclareOptionX*{\PassOptionsToPackage{\CurrentOption}{epycsdoc-base}}
\ProcessOptionsX\relax
% Load the base epycsdoc package
\RequirePackage{epycsdoc-base}
...
% URL: <http://epycsdoc.sf.net>
%
% This LaTeX stylesheet for epycsdoc's output uses shaded boxes to
...
% $Id:$
\NeedsTeXFormat{LaTeX2e}
\ProvidesClass{epycsdoc-shaded}[2008/02/26 v3.0.1 Epydoc Python Documentation]
% Pass options to the epycsdoc base package.
\RequirePackage{xkeyval}
\DeclareOptionX*{\PassOptionsToPackage{\CurrentOption}{epycsdoc-base}}
\ProcessOptionsX\relax
...
% epycsdoc-blue.sty
%
% A relatively minimal customization of a builtin epycsdoc style file,
% showing the basic pieces that need to be present.
%
\NeedsTeXFormat{LaTeX2e}
\ProvidesClass{epycsdoc-blue}[2008/02/26 v3.0.1 Epydoc Python Documentation]

% Load our base package (epycsdoc-shaded in this case).
\RequirePackage{xkeyval}
\DeclareOptionX*{\PassOptionsToPackage{\CurrentOption}{epycsdoc-shaded}}
\ProcessOptionsX\relax
\RequirePackage{epycsdoc-shaded}
...
TEMPLATE = r"""
% epycsdoc-template.sty
%
% This is a starting point for creating new epycsdoc style files.
...
\ProvidesClass{epycsdoc-XXX}[2008/02/26 v3.0.1 Epydoc Python Documentation]
