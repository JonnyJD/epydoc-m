...
C{epycsdoc.markup} package: it takes a docstring and the name of a
...
the documentation that epycsdoc generates.  C{DocstringLinker}s are
...
from epycsdoc.util import plaintext_to_html, plaintext_to_latex
import epycsdoc
...
_markup_language_registry = {
    'restructuredtext': 'epycsdoc.markup.restructuredtext',
    'epytext': 'epycsdoc.markup.epytext',
    'plaintext': 'epycsdoc.markup.plaintext',
    'javadoc': 'epycsdoc.markup.javadoc',
    }
...
def parse(docstring, markup='plaintext', errors=None, **options):
    ...
    if not re.match(r'\w+', markup):
        ...
        import epycsdoc.markup.plaintext as plaintext
...
class ParsedDocstring:
    ...
    of the documentation that epycsdoc generates.
    ...
    If and when epycsdoc adds more output formats, new C{to_I{format}}
...
class ParseError(Exception):
    ...
    def is_fatal(self):
        ...
        @return: true if this is a fatal error.  If an error is fatal,
            then epycsdoc should ignore the output of the parser, and
...
def parse_type_of(obj):
    ...
    from epycsdoc.markup.epytext import ParsedEpytextDocstring
