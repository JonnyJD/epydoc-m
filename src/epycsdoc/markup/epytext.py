#
# epytext.py: epycsdoc formatted docstring parsing
# Edward Loper
#
...
@var SYMBOLS: A list of the of escape symbols that are supported
      by epycsdoc.  Currently the following symbols are supported:
<<<SYMBOLS>>>
"""
...
from epycsdoc.markup import *
from epycsdoc.util import wordwrap, plaintext_to_html, plaintext_to_latex
...
def _add_list(doc, bullet_token, stack, indent_stack, errors):
    ...
    if newlist:
        ...
        if (list_type != 'fieldlist' and indent_stack[-1] is not None and
            ...
            # the comment-opening quote -- epycsdoc can't reliably
...
class ParsedEpytextDocstring(ParsedDocstring):
    ...
    def _build_graph(self, graph_type, graph_args, linker,
        ...
        if graph_type == 'classtree':
            from epycsdoc.apidoc import ClassDoc
            if graph_args:
                ...
                return None
            from epycsdoc.docwriter.dotgraph import class_tree_graph
            return class_tree_graph(bases, linker, context)
        elif graph_type == 'packagetree':
            from epycsdoc.apidoc import ModuleDoc
            if graph_args:
                ...
                return None
            from epycsdoc.docwriter.dotgraph import package_tree_graph
            ...
            from epycsdoc.docwriter.dotgraph import import_graph
            ...
            if graph_args:
                ...
                docs = [context]
            from epycsdoc.docwriter.dotgraph import call_graph
