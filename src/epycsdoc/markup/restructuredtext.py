...
from epycsdoc.apidoc import ModuleDoc, ClassDoc
...
#: recognized by epycsdoc; and whose values are the corresponding epycsdoc
...
class OptimizedReporter(docutils.utils.Reporter):
    ...
    shave a couple seconds off of epycsdoc's run time, since docutils
...
class _EpydocHTMLTranslator(HTMLTranslator):
    ...
    def starttag(self, node, tagname, suffix='\n', **attributes):
        ...
        tags, to prevent them from conflicting with epycsdoc.  In particular:
...
class dotgraph(docutils.nodes.image):
    ...
    epycsdoc has built; C{context} is the C{APIDoc} whose docstring
