#
# epycsdoc -- Utility functions used by regression tests (*.doctest)
# Edward Loper
#
...
from epycsdoc.docbuilder import build_doc, build_doc_index
from epycsdoc.docparser import parse_docs
from epycsdoc.docintrospecter import introspect_docs
from epycsdoc.apidoc import ClassDoc, RoutineDoc
from epycsdoc.markup import ParsedDocstring
...
def buildvaluedoc(s):
    ...
    val_doc = build_doc(os.path.join(tmp_dir, 'epycsdocc_test.py'))
...
def runbuilder(s, attribs='', build=None, exclude=''):
    ...
    s = re.sub(r"(<module 'epycsdocc_test' from ).*", r'\1...', s)
...
def runparser(s, attribs='', show=None, exclude=''):
    ...
    tmp_dir = write_pystring_to_tmp_dir(s)
    # Parse it.
    val_doc = parse_docs(os.path.join(tmp_dir, 'epycsdocc_test.py'))
    if show is not None:
...
def runintrospecter(s, attribs='', introspect=None, exclude=''):
    ...
    if introspect is None:
        import epycsdocc_test as val
    else:
        exec("from epycsdocc_test import %s as val" % introspect)
    del sys.path[0]
    # Introspect it.
...
def write_pystring_to_tmp_dir(s):
    tmp_dir = tempfile.mkdtemp()
    out = open(os.path.join(tmp_dir, 'epycsdocc_test.py'), 'w')
    out.write(textwrap.dedent(s))
    out.close()
    return tmp_dir

def cleanup_tmp_dir(tmp_dir):
    os.unlink(os.path.join(tmp_dir, 'epycsdocc_test.py'))
    try: os.unlink(os.path.join(tmp_dir, 'epycsdocc_test.pyc'))
    except OSError: pass
    os.rmdir(tmp_dir)
    sys.modules.pop('epycsdocc_test', None)
